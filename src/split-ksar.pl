...
sub modServerName {
    my($host) = @_;
    $host =~ s/(ncpla6t006)/$1-DPX_DGW#1/;
    $host =~ s/(ncpla6t007)/$1-DPX_DGW#2/;
    $host =~ s/(ncpla6t005)/$1-DPX_DGW#3/;
    $host =~ s/(ncpla6t045)/$1-DPX_DGW#4/;
    $host =~ s/(ncpla6t025)/$1-WFC#1/;
    $host =~ s/(ncpla6t017)/$1-WFC#2/;
    $host =~ s/(ncpla6t046)/$1-SV2/;
    $host =~ s/(ncpla6t018)/$1-DMR#1/;
    $host =~ s/(ncpla6t019)/$1-DMR#2/;
    return $host;
}
