...
print "修正前_15分wf_proc_data_id の値 : " . $pre_wf_proc_15 . "\n";
...
print "修正前_60分wf_proc_data_id の値 : " . $pre_wf_proc_60 . "\n";
...
# 値再設定
################################
if ($wf_proc_data_id_15 ne "") {
  `cp -p $file_15 $file_15.autobk `;

  open(my $fh_15, ">", $file_15) or die "Cannot open $file_15: $!";
  print $fh_15 $wf_proc_data_id_15;
  close($fh_15);
}

if ($wf_proc_data_id_60 ne "") {
  `cp -p $file_60 $file_60.autobk `;

  open(my $fh_60, ">", $file_60) or die "Cannot open $file_60: $!";
  print $fh_60 $wf_proc_data_id_60;
  close($fh_60);
}
