...
my $msr_time_15 = sprintf( "%04d-%02d-%02d %02d:%02d:%02d+0900", $year + 1900, $mon + 1, $mday, $hour, $min , 0);
print "    msr_time_15設定値           : $msr_time_15 \n";

# msr_time_60設定
my $msr_time_60 = sprintf( "%04d-%02d-%02d %02d:%02d:%02d+0900", $year + 1900, $mon + 1, $mday, $hour, 0 , 0);
print "    msr_time_60設定値           : $msr_time_60 \n";
# 前回時間を更新するかを判定。11分起動のときは更新する。
my $is_update_time_60 = "0";
my ($now_min , $now_hour)= (localtime(time))[1..2];
if ($now_hour != $hour){
    $is_update_time_60 = "11111";
}

my $command = "$JMETER_DIR/$JMETER_SH -n -Jcont_num=$cont_num -Jloop_repeat=$loop_repeat -Jpre_wf_proc_15=$pre_wf_proc_15 -Jpre_wf_proc_60=$pre_wf_proc_60 -Jis_update_time_60=$is_update_time_60 -Jmsr_time_15=\'$msr_time_15\' -Jmsr_time_60=\'$msr_time_60\' -t $JMX_DIR/$JMX_FILE -l $LOG_DIR/$LOD_FILE";
print "$command\n";
